...
a_3_2 = array([[a*sqrt(a) for a in sma]]).T
T = array([[0.2409, 0.6152, 1, 1.881, 11.86, 29.46, 84.01, 164.8, 247.7]]).T
...
X = array([[0.038, 0.194, 0.425, 0.6260, 1.2530, 2.5000, 3.7400]]).T
Y = array([[0.050, 0.127, 0.094, 0.2122, 0.2729, 0.2665, 0.3317]]).T

resf = lambda x, y, beta: y-array([beta[0]*xi/(beta[1]+xi) for xi in x])
drdb = lambda x, y, beta: array([[-xi/(beta[1]+xi) for xi in x],
                                 [beta[0]*xi/(beta[1]+xi)**2 for xi in x]]).T[0]
